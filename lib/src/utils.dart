...
import "dart:io";
import "dart:async";
import "package:dogma_codegen/build.dart" as dogma;
...
String get importApp => options?.name != null ? "import 'package:${options.name}/${options.name}.dart';" : "";

buildModel() {
  if (options?.name != null && options.settings["models"]?.library != null &&
      options.settings["models"]?.path != null) {

    dogma.build([], modelLibrary: "lib/${options.settings["models"].library}",
        modelPath: "lib/${options.settings["models"]?.path}",
        mapper: false,
        unmodifiable: false);
  }
}